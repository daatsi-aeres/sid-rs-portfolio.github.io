...
> **Bellman Equations**
\begin{equation}
 V_{\pi}(s) = \sum_{a \in \mathcal{A}}\pi(a|s) \left( \mathcal{R}(s,a) + \gamma \sum_{s' \in \mathcal{S}}\mathcal{P}(s' | s,a )V_{\pi}(s')\right)
 \end{equation}
 \begin{equation}
...
>$$ V^*(s) = \max_a Q^*(s, a) $$
>
>$$ Q^*(s,a) = \mathcal{R}(s,a) + \gamma\sum_{s' \in S} P(s' \mid s,a) V^*(s') $$
>
>$$ V^*(s) = \max_a \mathcal{R}(s,a) + \gamma\sum_{s' \in S} P(s' \mid s,a) V^*(s') $$
>
>$$ Q^* (s, a) = \mathcal{R}(s,a) + \gamma \sum_{s' \in S}P(s'\mid s,a) \max_{a'} Q^* (s', a')  $$
